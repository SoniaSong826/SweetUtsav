	...
	handleDelete = (i) => {
		...
		AsyncStorage.getItem('cart').then((cart) => {
			...
			for (let i in this.state.dataCart) {
				...
				addPrice =
					addPrice + parseFloat(this.state.dataCart[i].price) * parseFloat(this.state.dataCart[i].quantity);
			}
	...
	componentDidMount() {
		AsyncStorage.getItem('cart')
			.then((cart) => {
				...
				for (let i in this.state.dataCart) {
					const addAmount = this.state.totalAmount + parseInt(this.state.dataCart[i].quantity);
					const addPrice =
						this.state.totalPrice +
						parseFloat(this.state.dataCart[i].price) * parseFloat(this.state.dataCart[i].quantity);
					this.setState({ totalPrice: addPrice });
					this.setState({ totalAmount: addAmount });
				}
			})
			...
			<ImageBackground style={styles.backGround} source={require('../assets/lightOrange_background.jpg')}>
				...
				<View style={styles.buttomRound}>
					<View style={styles.textContainer}>
						<AppText style={styles.text}>Amount: {this.state.totalAmount}</AppText>
						<AppText style={styles.text}>Total: $ {Math.round(this.state.totalPrice * 100) / 100}</AppText>
					</View>
					<AppButton onPress={() => navigation.navigate('Address')} title="Check Out"></AppButton>
				</View>
			</ImageBackground>
		);
	}
